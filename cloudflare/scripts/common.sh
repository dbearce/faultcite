...
assert_config_ready() {
  ...
  local approved_hostname
  case "$config" in
    cloudflare/wrangler.staging.toml) approved_hostname="staging.faultcite.com" ;;
    cloudflare/wrangler.production.toml) approved_hostname="app.faultcite.com" ;;
    *) die "unrecognized Cloudflare config: $config" ;;
  esac
  [[ "$(grep -Ec '^\[\[routes\]\]$' "$config")" == 1 ]] || die "$config must have exactly one Custom Domain"
  grep -q "^pattern = \"$approved_hostname\"$" "$config" || die "$config Custom Domain is not approved"
  grep -q '^custom_domain = true$' "$config" || die "$config must use a Custom Domain"
