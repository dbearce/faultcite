...
function IntakeView(p: IntakeProps) {
  ...
  useEffect(() => { if (!p.error) return; const firstInvalid = errorRef.current?.closest(".analysis-main")?.querySelector<HTMLElement>('[aria-invalid="true"]'); (firstInvalid || errorRef.current)?.focus(); }, [p.error]);
...
function SearchModal({ close, machines, openMachine }: { close: () => void; machines: Machine[]; openMachine: (id: string) => void }) { const [query,setQuery]=useState(""); const dialogRef = useDialogFocus(close); const results=machines.filter(machine=>`${machine.asset} ${machine.manufacturer} ${machine.model} ${machine.location}`.toLowerCase().includes(query.toLowerCase())).slice(0,5); return <div className="modal-backdrop" role="presentation" onMouseDown={close}><div ref={dialogRef} className="modal search-modal" role="dialog" aria-modal="true" aria-labelledby="search-modal-title" onMouseDown={e => e.stopPropagation()}><h2 id="search-modal-title" className="sr-only">Search company machines</h2><div className="search-box"><Search /><input type="search" autoComplete="off" aria-label="Search company machines" aria-describedby="search-results-status" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search company machines…" /><button onClick={close} aria-label="Close search"><X /></button></div><small className="eyebrow">{query ? "MATCHING MACHINES" : "REGISTERED MACHINES"}</small><p id="search-results-status" className="sr-only" role="status">{`${results.length} ${results.length === 1 ? "machine" : "machines"} ${query ? "match this search" : "available"}.`}</p>{results.map(machine=><button key={machine.id} className="search-result" onClick={()=>openMachine(machine.id)}><span className="machine-image">{machine.image}</span><div><strong>{machine.asset} · {machine.manufacturer} {machine.model}</strong><p>{machine.location}</p></div><ChevronRight /></button>)}{!results.length&&<EmptyState title="No machine found" detail="Try an asset number, manufacturer, model, or location." />}</div></div>; }
