"use client";
/* eslint-disable react-hooks/purity -- live urgency and expiry labels intentionally reflect render time. */
...
type View = "home" | "intake" | "analysis" | "verify" | "complete" | "cases" | "machines" | "knowledge" | "impact" | "setup" | "history";
type Machine = {
...
const managerNav = [
  ...
  { id: "setup" as const, label: "Company setup", icon: UserCog },
];
...
export function TechnicianConsole({ signedInName, signedInEmail, signOutPath }: { signedInName: string; signedInEmail: string; signOutPath: string }) {
  ...
  const initialWorkspaceModeChosen = useRef(false);
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    try {
      ...
      if (!initialWorkspaceModeChosen.current) {
        initialWorkspaceModeChosen.current = true;
        if (["owner", "manager"].includes(payload.user.role)) {
          setRole("manager");
          setView(payload.user.role === "owner" && payload.machines.length === 0 ? "setup" : "home");
        }
      }
  ...
  return <div className="shell">
    ...
    <main id="faultcite-main" ref={mainRef} tabIndex={-1} aria-label={`${view.replaceAll("_", " ")} screen`}>
      ...
      {!workspaceError && view === "machines" && <UtilityView title="Machine registry" eyebrow={`${machines.length} ASSETS`} icon={<Gauge />}>{canManage && role === "manager" && <MachineAdmin onSaved={machine => setMachines(current => [...current, machine])} />}{machines.length ? <><div className="registry">{machines.map(m => <MachineCard key={m.id} machine={m} onClick={() => openMachine(m.id)} />)}</div>{canManage&&role==="manager"&&<MachineCorrection machines={machines} onSaved={machine=>setMachines(current=>current.map(item=>item.id===machine.id?machine:item))}/>}</> : <EmptyState title="No machines registered" detail={canManage && role === "manager" ? "Use the registration form above to add the first company machine." : canManage ? "Switch to Manager tasks to register the first company machine." : "Ask a manager to add the first company machine."} />}</UtilityView>}
      ...
      {!workspaceError && view === "knowledge" && <UtilityView title="Company manuals" eyebrow={`${manualRecords.length} DOCUMENTS · ${manualSources.length} APPROVED SOURCES`} icon={<BookOpen />}>{canManage && role === "manager" && <ManualUploadPanel onSaved={manual => setManualRecords(current => [manual, ...current])} />}<KnowledgeList manuals={manualRecords} machines={machines} sources={manualSources} canManage={canManage && role === "manager"} onChanged={setManualRecords} onSourcesRevoked={manualId => setManualSources(current => current.filter(source => source.manualId !== manualId))} onSourceApproved={source => setManualSources(current => [source, ...current])} /></UtilityView>}
      ...
      {!workspaceError && view === "setup" && workspaceRole === "owner" && <OwnerSetup />}
    </main>
...
function CompanySwitcher({ current, organizations, canCreate, canAdminister, disabled }: { current: Organization; organizations: Organization[]; canCreate: boolean; canAdminister: boolean; disabled: boolean }) {
  ...
  async function switchCompany(organizationId: string) {
    ...
    try {
      const response = await fetch("/api/organizations", { method: "PATCH", headers: { "content-type": "application/json" }, signal: AbortSignal.timeout(15_000), body: JSON.stringify({ organizationId }) });
      if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; return setMessage(payload?.error || "Company could not be opened."); }
      window.location.reload();
    } catch { setMessage("Company could not be opened. Check the connection and try again."); }
  ...
  async function createCompany(event: FormEvent) {
    ...
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/organizations", { method: "POST", headers: { "content-type": "application/json" }, signal: AbortSignal.timeout(15_000), body: JSON.stringify({ name }) });
      if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; return setMessage(payload?.error || "Company could not be created."); }
      window.location.reload();
    } catch { setMessage("Company could not be created. Check the connection and try again."); }
    finally { setBusy(false); }
  }
  async function renameCompany() { const next=window.prompt("Company name",current.name)?.trim(); if(!next||next===current.name)return;setBusy(true);setMessage("");try{const response=await fetch("/api/organizations",{method:"PATCH",headers:{"content-type":"application/json"},signal:AbortSignal.timeout(15_000),body:JSON.stringify({action:"rename",name:next})});if(!response.ok)return setMessage("Company name could not be changed.");window.location.reload();}catch{setMessage("Company name could not be changed. Check the connection and try again.");}finally{setBusy(false);} }
  return <section className="company-switcher" aria-label="Company administration">
    ...
    {canCreate && <button type="button" disabled={disabled} className="company-add" aria-expanded={creating} aria-controls="new-company-form" onClick={() => { setCreating(value => !value); setMessage(""); }}><Plus />{creating ? "Cancel" : "Add company"}</button>}
...
function ManagerOperations({ go, machines, cases, manuals, manualSources, team, workspaceRole, currentUserId }: { go: (v: View) => void; machines: Machine[]; cases: CaseRecord[]; manuals: ManualRecord[]; manualSources: ManualSourceRecord[]; team: TeamMember[]; workspaceRole: string; currentUserId: string }) {
  ...
  const activeApprovers=team.filter(item=>item.active&&item.userId!==currentUserId&&["owner","manager"].includes(item.role));
...
type IntakeProps = {
  ...
  notes: string; setNotes: (v: string) => void; error: string; setError: (v: string) => void; submitIntake: (e: FormEvent) => void; back: () => void;
...
function IntakeView(p: IntakeProps) {
  ...
  useEffect(() => { if (!p.error) return; const firstInvalid = errorRef.current?.closest("form")?.querySelector<HTMLElement>('[aria-invalid="true"]'); (firstInvalid || errorRef.current)?.focus(); }, [p.error]);
  ...
  return <div className="page focused">
    ...
    <div className="analysis-layout"><section className="analysis-main">
      <article className="check-card current-check"><div className="check-label"><span>TECHNICIAN OBSERVATION {p.checkNumber}</span><b>SOURCE-GATED MODE</b></div>{p.sources.length ? <div className="approved-guidance"><ShieldCheck /><span><strong>{p.sources.length} approved manual {p.sources.length === 1 ? "source applies" : "sources apply"} to this machine{p.alarm ? " and alarm" : ""}.</strong><small>Review the exact cited pages and safety notes before recording independently authorized work.</small></span><button className="secondary" type="button" onClick={p.openSource}>View approved sources</button></div> : <div className="confidence-warning"><AlertTriangle /><span><strong>No approved, applicable manual page is indexed for this case.</strong><small>FaultCite will not invent a procedure, expected result, likely cause, or citation.</small></span><button className="secondary" type="button" onClick={p.openSource}>Why guidance is withheld</button></div>}<div className="check-name"><span><ClipboardCheck /></span><div><h2>Record only work you are independently authorized and qualified to perform</h2><p>{p.sources.length ? "Use the approved source alongside your employer's safety program. FaultCite records the observation but does not authorize the work." : "Use your employer's safety program and the correct OEM documentation outside FaultCite. This record does not authorize a diagnostic step."}</p></div></div>
        {!p.resultSaved && <><div className="diagnostic-structure"><div className="field"><label htmlFor="suspected-cause">Suspected cause being tested <b>Required</b></label><input id="suspected-cause" required aria-invalid={Boolean(p.error && !p.suspectedCause.trim())} aria-describedby={p.error && !p.suspectedCause.trim() ? "diagnostic-error" : undefined} value={p.suspectedCause} onChange={e => p.setSuspectedCause(e.target.value)} placeholder="Example: low 24 VDC at the I/O module under load" /></div><div className="field"><label htmlFor="test-performed">Authorized test performed <b>Required</b></label><textarea id="test-performed" required aria-invalid={Boolean(p.error && !p.testPerformed.trim())} aria-describedby={p.error && !p.testPerformed.trim() ? "diagnostic-error" : undefined} value={p.testPerformed} onChange={e => p.setTestPerformed(e.target.value)} placeholder="Describe the approved test or inspection actually performed. Do not record a planned unsafe step." /></div><div className="two-inputs"><div className="field"><label htmlFor="expected-result">Expected result <b>Required</b></label><input id="expected-result" required aria-invalid={Boolean(p.error && !p.expectedResult.trim())} aria-describedby={p.error && !p.expectedResult.trim() ? "diagnostic-error" : undefined} value={p.expectedResult} onChange={e => p.setExpectedResult(e.target.value)} placeholder="Expected value or condition from the approved source" /></div><div className="field"><label htmlFor="units">Units</label><input id="units" value={p.units} onChange={e => p.setUnits(e.target.value)} placeholder="VDC, psi, mm, °F…" /></div></div></div><div className="result-label" id="result-label">What does the actual observation mean for this named cause?</div><div className="result-buttons" role="group" aria-labelledby="result-label" aria-describedby={p.error && !p.result ? "diagnostic-error" : undefined}>{(["Supports suspected cause", "Does not support suspected cause", "Unable to test", "Unsafe — escalate"] as Result[]).map(r => <button type="button" key={r} aria-pressed={p.result === r} className={p.result === r ? "chosen" : ""} onClick={() => p.setResult(r)}>{r}</button>)}</div>{p.result && <div className="result-capture"><div className="field"><label htmlFor="reading">Actual reading or observation <b>Required</b></label><input id="reading" required aria-invalid={Boolean(p.error && !p.reading.trim())} aria-describedby={p.error && !p.reading.trim() ? "diagnostic-error" : undefined} value={p.reading} onChange={e => p.setReading(e.target.value)} placeholder="Record exactly what was observed and how it was measured" /></div><button className="primary" type="button" disabled={p.resultSaving} onClick={p.saveResult}>{p.resultSaving ? "Saving…" : "Save structured observation"}</button></div>}</>}
        ...
        {p.error && <div ref={errorRef} tabIndex={-1} id="diagnostic-error" className="error" role="alert"><AlertTriangle />{p.error}</div>}
      </article>
...
function VerificationView({ machine, result, caseId, draft, confirmedCause, signedInName, canApprove, evidenceCount, uploadCloseoutEvidence, back: returnToDiagnostic, submitted, complete }: { machine: Machine; result: Result | null; caseId: string; draft?: CaseRecord; confirmedCause: string; signedInName: string; canApprove: boolean; evidenceCount: number; uploadCloseoutEvidence: (file: File) => Promise<void>; back: () => void; submitted: (casePatch: Partial<CaseRecord>) => void; complete: (casePatch: Partial<CaseRecord>) => void }) {
  ...
  const closeoutFormRef = useRef<HTMLFormElement>(null); const closeoutErrorRef = useRef<HTMLDivElement>(null);
  ...
  const missingCloseoutItems = [!caseId && "saved case", !confirmedCause && "manager-confirmed cause", !work.trim() && "work performed", !beforeAfter.trim() && "before and after readings", !cycles.trim() && "successful test cycles", !safety && "safety-device verification", !temporaryReady && "temporary-repair controls", !review && "closeout review confirmation", canApprove && (!authorized || !approval) && "restart authorization"].filter(Boolean) as string[];
  const canSubmit = Boolean(!saving && !photoUploading && missingCloseoutItems.length === 0);
  ...
  useEffect(() => { if (!error) return; const firstInvalid = closeoutFormRef.current?.querySelector<HTMLElement>('[aria-invalid="true"]'); (firstInvalid || closeoutErrorRef.current)?.focus(); }, [error]);
...
function MachineAdmin({ onSaved }: { onSaved: (machine: Machine) => void }) {
  ...
  return <form className="form-card team-panel" onSubmit={save}><div className="utility-title"><span><Gauge /></span><div><small className="eyebrow">MANAGER CONTROL</small><h2>Register a machine</h2><p>Technicians must verify these identity fields against the machine nameplate.</p></div></div><div className="two-inputs"><div className="field"><label htmlFor="asset-number">Asset number</label><input id="asset-number" required value={assetNumber} onChange={e=>setAssetNumber(e.target.value)} /></div><div className="field"><label htmlFor="manufacturer">Manufacturer</label><input id="manufacturer" required value={manufacturer} onChange={e=>setManufacturer(e.target.value)} /></div><div className="field"><label htmlFor="model">Model</label><input id="model" required value={model} onChange={e=>setModel(e.target.value)} /></div><div className="field"><label htmlFor="serial-number">Serial number</label><input id="serial-number" value={serialNumber} onChange={e=>setSerialNumber(e.target.value)} /></div><div className="field"><label htmlFor="control">CNC control</label><input id="control" value={control} onChange={e=>setControl(e.target.value)} /></div><div className="field"><label htmlFor="location">Location</label><input id="location" value={location} onChange={e=>setLocation(e.target.value)} /></div></div><button className="primary" disabled={saving}>{saving?"Saving…":"Add machine"}</button>{message&&<p className="fine-print" role="status">{message}</p>}</form>;
}

function MachineCorrection({machines,onSaved}:{machines:Machine[];onSaved:(machine:Machine)=>void}){
  const [selected,setSelected]=useState(""); const machine=machines.find(item=>item.id===selected); const [message,setMessage]=useState(""); const [saving,setSaving]=useState(false);
  async function save(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!machine)return;const form=new FormData(event.currentTarget);setSaving(true);setMessage("");try{const body={id:machine.id,assetNumber:form.get("assetNumber"),manufacturer:form.get("manufacturer"),model:form.get("model"),serialNumber:form.get("serialNumber"),control:form.get("control"),location:form.get("location")};const response=await fetch("/api/machines",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(body),signal:AbortSignal.timeout(15_000)});const payload=await response.json().catch(()=>null) as {machine?:{id:string;assetNumber:string;manufacturer:string;model:string;serialNumber:string|null;control:string|null;location:string|null;status:string};error?:string}|null;if(!response.ok||!payload?.machine)throw new Error(payload?.error||"Machine correction was not saved.");const row=payload.machine;onSaved({id:row.id,asset:row.assetNumber,manufacturer:row.manufacturer,model:row.model,serial:row.serialNumber||"Not recorded",control:row.control||"Not recorded",location:row.location||"Location not recorded",status:(["down","attention","running"].includes(row.status)?row.status:"attention") as Machine["status"],image:`${row.manufacturer[0]||"C"}${row.model[0]||"M"}`.toUpperCase()});setMessage("Machine identity correction saved in the audit trail. Earlier exact-page approvals were revoked for manager re-review.");}catch(error){setMessage(error instanceof Error?error.message:"Machine correction was not saved.");}finally{setSaving(false);}}
  return <section className="form-card team-panel"><div className="utility-title"><span><Gauge/></span><div><small className="eyebrow">CONTROLLED CORRECTION</small><h2>Correct machine identity</h2><p>Corrections are audited and blocked while the machine has an active case. Saving a correction revokes earlier exact-page source approvals so a manager can review the corrected identity.</p></div></div><div className="field"><label htmlFor="correct-machine">Machine to correct</label><select id="correct-machine" value={selected} onChange={event=>{setSelected(event.target.value);setMessage("");}}><option value="">Choose a machine…</option>{machines.map(item=><option key={item.id} value={item.id}>{item.asset} · {item.manufacturer} {item.model}</option>)}</select></div>{machine&&<form key={machine.id} onSubmit={save}><div className="two-inputs"><div className="field"><label>Asset number<input name="assetNumber" required defaultValue={machine.asset}/></label></div><div className="field"><label>Manufacturer<input name="manufacturer" required defaultValue={machine.manufacturer}/></label></div><div className="field"><label>Model<input name="model" required defaultValue={machine.model}/></label></div><div className="field"><label>Serial number<input name="serialNumber" defaultValue={machine.serial==="Not recorded"?"":machine.serial}/></label></div><div className="field"><label>CNC control<input name="control" defaultValue={machine.control==="Not recorded"?"":machine.control}/></label></div><div className="field"><label>Location<input name="location" defaultValue={machine.location==="Location not recorded"?"":machine.location}/></label></div></div><button className="primary" disabled={saving}>{saving?"Saving correction…":"Save audited correction"}</button></form>}{message&&<p className="fine-print" role="status">{message}</p>}</section>;
}

type OwnerOrganization={reviewSlaMinutes:number;dataRetentionDays:number;safetyContactEmail:string|null;supportContactEmail:string|null;termsAcceptedAt:string|number|null};
function OwnerSetup(){const [organization,setOrganization]=useState<OwnerOrganization|null>(null);const [message,setMessage]=useState("");const [saving,setSaving]=useState(false);useEffect(()=>{const controller=new AbortController();fetch("/api/settings",{cache:"no-store",signal:controller.signal}).then(async response=>{const payload=await response.json() as {organization?:OwnerOrganization;error?:string};if(!response.ok||!payload.organization)throw new Error(payload.error||"Company setup could not be loaded.");setOrganization(payload.organization);}).catch(error=>{if(!controller.signal.aborted)setMessage(error instanceof Error?error.message:"Company setup could not be loaded.");});return()=>controller.abort();},[]);async function save(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=new FormData(event.currentTarget);setSaving(true);setMessage("");try{const response=await fetch("/api/settings",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({reviewSlaMinutes:Number(form.get("reviewSlaMinutes")),dataRetentionDays:Number(form.get("dataRetentionDays")),safetyContactEmail:form.get("safetyContactEmail"),supportContactEmail:form.get("supportContactEmail"),termsAccepted:form.get("termsAccepted")==="on"})});const payload=await response.json().catch(()=>null) as OwnerOrganization&{error?:string};if(!response.ok)throw new Error(payload?.error||"Company setup was not saved.");setOrganization(payload);setMessage("Company governance settings saved and added to the audit trail.");}catch(error){setMessage(error instanceof Error?error.message:"Company setup was not saved.");}finally{setSaving(false);}}return <UtilityView title="Company setup" eyebrow="OWNER CONTROL" icon={<UserCog/>}>{!organization?<p role="status">{message||"Loading company governance…"}</p>:<form className="form-card team-panel" onSubmit={save}><h2>Safety, review, and record governance</h2><p>These settings control the readiness gate shown to your company.</p><div className="two-inputs"><div className="field"><label htmlFor="safety-contact">Safety contact email</label><input id="safety-contact" name="safetyContactEmail" type="email" required defaultValue={organization.safetyContactEmail||""}/></div><div className="field"><label htmlFor="support-contact">Support contact email</label><input id="support-contact" name="supportContactEmail" type="email" required defaultValue={organization.supportContactEmail||""}/></div><div className="field"><label htmlFor="review-sla">Manager review target (minutes)</label><input id="review-sla" name="reviewSlaMinutes" type="number" min="15" max="10080" required defaultValue={organization.reviewSlaMinutes}/></div><div className="field"><label htmlFor="retention-days">Record retention (days)</label><input id="retention-days" name="dataRetentionDays" type="number" min="365" max="3650" required defaultValue={organization.dataRetentionDays}/></div></div><label className="check"><input name="termsAccepted" type="checkbox" required defaultChecked={Boolean(organization.termsAcceptedAt)}/>I acknowledge that qualified people, employer safety procedures, LOTO, and approved OEM information remain authoritative.</label><button className="primary" disabled={saving}>{saving?"Saving governance…":"Save company setup"}</button>{message&&<p className="fine-print" role="status">{message}</p>}</form>}</UtilityView>}
...
function KnowledgeList({ manuals, machines, sources, canManage, onChanged, onSourcesRevoked, onSourceApproved }: { manuals: ManualRecord[]; machines: Machine[]; sources: ManualSourceRecord[]; canManage: boolean; onChanged: (manuals: ManualRecord[]) => void; onSourcesRevoked: (manualId: string) => void; onSourceApproved: (source: ManualSourceRecord) => void }) {
  ...
  return <><div className="knowledge-list">{manuals.map(manual=>{const citations=sources.filter(source=>source.manualId===manual.id);const expired=!manual.revalidationDueAt||new Date(manual.revalidationDueAt).valueOf()<=Date.now();return <article key={manual.id} className="manual-record"><div className="manual-row"><span><FileText /></span><div><small>{manual.manufacturer.toUpperCase()} · {(manual.model || "ALL MODELS").toUpperCase()}</small><h2>{manual.title}</h2><p>{manual.revision ? `Revision ${manual.revision}` : "Revision not recorded"} · {manual.serialApplicability || "Serial applicability not recorded"} · {manual.pageCount ? `${manual.pageCount} PDF pages verified` : "PDF page count not verified"}</p><p className="manual-state">{expired?"Revalidation required — blocked from diagnostic use":manual.status==="approved"?citations.length?`${citations.length} exact source ${citations.length===1?"record":"records"} approved`:"Metadata approved · exact-page review pending":manual.status.replaceAll("_"," ")}</p></div><div className="manual-actions"><a className="secondary" href={`/api/manuals/${manual.id}`} target="_blank" rel="noreferrer">Open PDF</a>{canManage&&expired&&<button type="button" className="secondary" onClick={()=>{const date=window.prompt("New revalidation date after today (YYYY-MM-DD)");if(date)void review(manual,"approved",date);}}>Revalidate manual</button>}{canManage&&!expired&&manual.status!=="approved"&&<button type="button" className="secondary" onClick={()=>review(manual,"approved")}>Approve metadata</button>}{canManage&&manual.status!=="rejected"&&<button type="button" className="secondary" onClick={()=>review(manual,"rejected")}>Reject</button>}</div></div>{citations.length>0&&<div className="approved-source-list">{citations.map(source=><div key={source.id} className="approved-source"><ShieldCheck /><div><strong>{source.sectionTitle} · {source.pageStart===source.pageEnd?`page ${source.pageStart}`:`pages ${source.pageStart}–${source.pageEnd}`}</strong><p>{machines.find(machine=>machine.id===source.machineId)?.asset || `${source.manufacturer} ${source.model}`} · {source.alarmCode?`Alarm ${source.alarmCode}`:"All alarms for this machine"}</p><small>{source.sourceSummary}</small></div></div>)}</div>}{canManage&&manual.status==="approved"&&!expired&&<SourceApprovalForm manual={manual} machines={machines} onApproved={onSourceApproved} />}</article>})}</div>{message&&<p className="fine-print" role="status">{message}</p>}</>;
}
...
function SourceModal({ close, machine, alarmCode }: { close: () => void; machine: Machine; alarmCode: string }) {
  const dialogRef = useDialogFocus(close); const [sources,setSources]=useState<ManualSourceRecord[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [reload,setReload]=useState(0);
  useEffect(()=>{const controller=new AbortController();void (async()=>{setLoading(true);setError("");try{const response=await fetch(`/api/manual-sources?machineId=${encodeURIComponent(machine.id)}&alarmCode=${encodeURIComponent(alarmCode)}`,{cache:"no-store",signal:AbortSignal.any([controller.signal,AbortSignal.timeout(15_000)])});const payload=await response.json().catch(()=>null) as {sources?:ManualSourceRecord[];error?:string}|null;if(!response.ok)throw new Error(payload?.error||"Approved sources could not be checked.");setSources(payload?.sources||[]);}catch(failure){if(!controller.signal.aborted)setError(failure instanceof Error&&failure.name!=="TimeoutError"?failure.message:"Approved sources did not respond. Check the connection and try again.");}finally{if(!controller.signal.aborted)setLoading(false);}})();return()=>controller.abort();},[alarmCode,machine.id,reload]);
  return <div className="modal-backdrop" role="presentation" onMouseDown={close}><div ref={dialogRef} className="modal source-modal" role="dialog" aria-modal="true" aria-labelledby="source-modal-title" aria-busy={loading} onMouseDown={e => e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">REVIEWED OEM EVIDENCE</span><h2 id="source-modal-title">Applicable manual sources</h2></div><button type="button" onClick={close} aria-label="Close source status"><X /></button></div>{loading?<div className="manual-page" role="status"><p>Checking approved pages for {machine.asset}…</p></div>:error?<div className="timeline-error" role="alert"><strong>Approved sources unavailable</strong><p>{error}</p><button className="secondary full" type="button" onClick={()=>setReload(value=>value+1)}>Try again</button></div>:sources.length?<div className="source-citations">{sources.map(source=><article key={source.id} className="manual-page"><header>{source.manualTitle || "Approved company manual"}{source.manualRevision?` · Revision ${source.manualRevision}`:""}</header><h3>{source.sectionTitle}</h3><div className="source-meta"><span><FileText />{source.pageStart===source.pageEnd?`Page ${source.pageStart}`:`Pages ${source.pageStart}–${source.pageEnd}`}</span><span><Gauge />{machine.asset} · {source.manufacturer} {source.model}</span>{source.alarmCode&&<span><AlertTriangle />Alarm {source.alarmCode}</span>}</div><p>{source.sourceSummary}</p><p className="manual-warning"><ShieldAlert />{source.safetyNotes}</p><a className="secondary" href={`/api/manuals/${source.manualId}#page=${source.pageStart}`} target="_blank" rel="noreferrer">Open reviewed PDF page</a><footer>Manager-approved source · {new Date(source.approvedAt).toLocaleDateString()}</footer></article>)}</div>:<div className="manual-page"><p>No manager-approved page matches this exact registered machine{alarmCode?` and alarm ${alarmCode}`:""}.</p><p className="manual-warning"><AlertTriangle />FaultCite will not invent a procedure, expected result, likely cause, or citation. Use the correct OEM documentation and your employer&apos;s safety program.</p></div>}<button className="primary full" type="button" onClick={close}>Return to case</button></div></div>;
}
function SearchModal({ close, machines, openMachine }: { close: () => void; machines: Machine[]; openMachine: (id: string) => void }) { const [query,setQuery]=useState(""); const dialogRef = useDialogFocus(close); const results=machines.filter(machine=>`${machine.asset} ${machine.manufacturer} ${machine.model} ${machine.location}`.toLowerCase().includes(query.toLowerCase())).slice(0,5); return <div className="modal-backdrop" role="presentation" onMouseDown={close}><div ref={dialogRef} className="modal search-modal" role="dialog" aria-modal="true" aria-labelledby="search-modal-title" onMouseDown={e => e.stopPropagation()}><h2 id="search-modal-title" className="sr-only">Search company machines</h2><div className="search-box"><Search /><input type="search" autoComplete="off" aria-label="Search company machines" aria-describedby="search-results-status" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search company machines…" /><button type="button" onClick={close} aria-label="Close search"><X /></button></div><small className="eyebrow">{query ? "MATCHING MACHINES" : "REGISTERED MACHINES"}</small><p id="search-results-status" className="sr-only" role="status">{`${results.length} ${results.length === 1 ? "machine" : "machines"} ${query ? "match this search" : "available"}.`}</p>{results.map(machine=><button type="button" key={machine.id} className="search-result" onClick={()=>openMachine(machine.id)}><span className="machine-image">{machine.image}</span><div><strong>{machine.asset} · {machine.manufacturer} {machine.model}</strong><p>{machine.location}</p></div><ChevronRight /></button>)}{!results.length&&<EmptyState title="No machine found" detail="Try an asset number, manufacturer, model, or location." />}</div></div>; }
