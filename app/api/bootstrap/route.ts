...
export async function GET() {
  ...
  const [[organization], machineRows, activeCaseRows, historyCaseRows, manualRows, sourceRows, teamRows, inviteRows, workspaceRows] = await Promise.all([
    ...
    db.select({ id: manualSources.id, manualId: manualSources.manualId, machineId: manualSources.machineId, manufacturer: manualSources.manufacturer, model: manualSources.model, serialNumber: manualSources.serialNumber, alarmCode: manualSources.alarmCode, sectionTitle: manualSources.sectionTitle, pageStart: manualSources.pageStart, pageEnd: manualSources.pageEnd, sourceSummary: manualSources.sourceSummary, safetyNotes: manualSources.safetyNotes, approvedAt: manualSources.approvedAt }).from(manualSources).innerJoin(manuals, eq(manualSources.manualId, manuals.id)).where(and(eq(manualSources.organizationId, ctx.organizationId), isNull(manualSources.revokedAt), eq(manuals.organizationId, ctx.organizationId), eq(manuals.status, "approved"), gt(manuals.revalidationDueAt, new Date()))),
