...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return apiError("Invalid JSON request");
  }
  ...
  if (existingClose) return Response.json({ caseId: id, status: "closed", closedAt: record.closedAt, approvedBy: ctx.displayName, case: record, idempotentReplay: true });
  if (record.status !== "closeout_requested") return apiError("A technician closeout submission is required before manager approval", 409);
  if (!record.closeoutSubmittedByUserId) return apiError("The closeout submitter is missing. Return the record to a technician before approval.", 409);
  if (record.closeoutSubmittedByUserId === ctx.userId) return apiError("A different authorized manager or owner must record restart authorization", 409);
  ...
  try {
    ...
    const repairSummary = record.repairSummary;
    const verificationReadings = record.verificationReadings;
    const testCycles = record.testCycles;
    const repairType = record.repairType;
    if (!repairSummary || !verificationReadings || !testCycles || !new Set(["Permanent", "Temporary"]).has(repairType || "")) return apiError("The submitted closeout record is incomplete", 409);
    if (!record.safetyDevicesVerified || body.approvalConfirmed !== true) return apiError("Safety verification and authenticated approval are required");

    const followupWork = record.followupWork;
    const restrictions = record.operatingRestrictions;
    const expires = record.temporaryExpiresAt;
    ...
    const casePatch = { status: "closed", restartApprovedByUserId: ctx.userId, managerActionDueAt: null, closedAt: now, updatedAt: now };
    try { await db.batch([
      db.update(cases).set(casePatch).where(and(eq(cases.id, id), eq(cases.organizationId, ctx.organizationId), eq(cases.status, record.status))),
      ...
      db.insert(auditLogs).values({ id: crypto.randomUUID(), organizationId: ctx.organizationId, actorUserId: ctx.userId, action: "case.closed", entityType: "case", entityId: id, metadataJson: JSON.stringify({ repairType, closeoutSubmittedByUserId: record.closeoutSubmittedByUserId, restartApprovedByUserId: ctx.userId, followupCaseId: followupId, closeEventId }), createdAt: now }),
    ]); } catch {
      const [current] = await db.select().from(cases).where(and(eq(cases.id, id), eq(cases.organizationId, ctx.organizationId))).limit(1);
      if (current?.status === "closed") return Response.json({ caseId: id, status: "closed", closedAt: current.closedAt, case: current, idempotentReplay: true });
      return apiError("Case state changed before closeout. Refresh and review it again.", 409);
    }
    return Response.json({ caseId: id, status: "closed", closedAt: now.toISOString(), approvedBy: ctx.displayName, case: { ...record, ...casePatch }, followupCase: followupId ? { id: followupId, caseNumber: followupNumber } : null });
  } catch (error) {
